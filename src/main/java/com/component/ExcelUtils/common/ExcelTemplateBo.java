...
public class ExcelTemplateBo {
    ...
    private ExcelTemplateBo() {
    }

    public static ExcelTemplateBo getInstance() {
        return new ExcelTemplateBo();
    }

    public ExcelTemplateBo readTemplateByClasspath(String path,int size) {
        try {
            this.wb = new HSSFWorkbook(TemplateFileUtil.getTemplates(path));
            System.out.println(size);
            this.initTemplate(size); //在这方法里面进行表头标题合并
            return this;
        } catch (IOException var3) {
            var3.printStackTrace();
            throw new RuntimeException("读取模板不存在,请检查！ path:"+path);
        }
    }

    public ExcelTemplateBo readTemplateByPath(String path) {
        try {
    ...
    private void initTemplate(int size) {
        this.sheet = this.wb.getSheetAt(0);
        this.initConfigData();
        this.lastRowIndex = this.sheet.getLastRowNum();
        this.curRow = this.sheet.createRow(this.curRowIndex);
        this.sheet.addMergedRegion(new CellRangeAddress(0,0,0,size-1)); //此处合并
    }
